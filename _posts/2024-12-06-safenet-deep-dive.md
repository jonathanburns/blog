...
_The protocol allows the processor to extract the funds on the debit chain at any time if they prove on the debit chain that the funds were delivered. However, as described previously, most proving methods tend to be expensive and time-consuming, and so waiting for the withdrawal period works better in practice._
